...
from my_work.dynamics import *
from my_work.data_loader import *

from my_work.data_loader import JointData, C3DData
...
class ReactionPlotter:
    ...
    def __init__(self, ax, reaction_data, position_data, time_delay = 0, freq=100,
        ...
        self.reaction_data, self.position_data = reaction_data[frame_delay : frame_delay + len(position_data)], position_data
...
class ModelPlotter(JointData):
    ...
    def __init__(self, ax, data, body_joints, freq=60, norm=False, bone_dict = {}):
        ...
        self.cylinders = [Cylinder(ax, radius=0.05, start=Vector(0,0,0), end=Vector(0.1,0.1,0.1), name=bone) for bone in self.bones]
    ...
    def update_frame(self, n_frame=0):
        ...
        start, end = Vector(*np.mean(self.all_data[n_frame, self.body_joints[0]], axis=0)), Vector(*np.mean(self.all_data[n_frame, self.body_joints[1]], axis=0))
...
def kinematics_with_reaction_forces(raw_mocap_src = "", raw_reactions_src = "", reactions_time_delay=0,
    ...
    axes = gridspec.GridSpec(1, n_subplots)
    ...
    if is_mesh_data:
        ax_mesh = fig.add_subplot(axes[0, int(is_raw_data)], projection = "3d") #Mesh is axis 0 if no raw data, 1 otherwise
        existing_axes.append(ax_mesh)
        mesh_plot = MeshPlotter(ax_mesh, mesh_data.vertex_data, mesh_data.face_data)

    if is_predicted_data:
        ax_predicted = fig.add_subplot(axes[0, int(is_raw_data) + int(is_mesh_data)], projection = "3d") # Similar to above
    ...
    if is_raw_data:
        ...
        if is_raw_reactions:
            ...
            raw_reaction_plotter = ReactionPlotter(ax_raw, reaction_data=raw_reaction_data, position_data=reaction_positions, time_delay = t_delay,
                mutation_scale=20, lw=3, arrowstyle="-|>", color="r", baseline_weight=250, arrow_scale=(z_bounds[1] - z_bounds[0])/5)

    if is_predicted_data:
        # print(predicted_skeleton.shape)
        # print(BoneMappings().get_mocap_mapping(mocap_data)[0])
        predicted_skeleton_plotter = ModelPlotter(ax_predicted, mocap_data.all_data, freq = 60,
                                                  ...
                                                  body_joints = BoneMappings().get_mocap_mapping(mocap_data)[1]) # Review frequency here

        # Set bounds
        x_bounds, y_bounds, z_bounds = [buffer(*getattr(predicted_skeleton_plotter, f"{dim}_bounds"), val=0.01) for dim in
                                        "xyz"]  # Get all dimensions with a buffer of 10
        ax_predicted.set_xlim(*x_bounds)
        ax_predicted.set_ylim(*y_bounds)
        ax_predicted.set_zlim(*z_bounds)

        predicted_reactions_plotter = ReactionPlotter(ax_predicted, *predicted_reactions, freq = predicted_skeleton_plotter.freq)
    ...
    plt.tight_layout()
    save_animation(anim, fig, frames = 100, dir=r"C:\Users\Ollie\Videos\iib_project\dynamics_overview", fps=fps * playback_speed)
...
def mocap_and_forceplate_time_series(mocap_src = r"set_1 - 14-10-19/3kph capture 4.c3d",
    ...
    n_frames_forceplate = int(len(reaction_positions) * freq_forceplate/freq_mocap) # number of frames for forceplate to be same time length as mocap
...
def compare_predicted_forcing_time_series(mocap_src =r"set_1 - 14-10-19/3kph capture 4.c3d",
    ...
    try: load_force_plate_data(actual_reactions_src)
    except FileNotFoundError: ForcePlateData(src=actual_reactions_src.replace(" -- ", r"/")) # Load and save force data if not found (making sure to change the filename to a valid directory type)

    actual_reactions, t_delay = load_force_plate_data(actual_reactions_src)

    frame_delay = int(freq_forceplate * t_delay)
    n_frames_forceplate = int(len(predicted_mocap_reactions) * freq_forceplate/freq_mocap) # number of frames for forceplate to be same time length as mocap

    actual_reactions = actual_reactions[frame_delay: frame_delay + n_frames_forceplate] # crop forceplate data to match mocap/SMAL data
    ...
    for n_paw in range(4):
        ...
        ax.plot(t_range_mocap[50:], norm(predicted_mocap_reactions[:, n_paw][50:]), color="blue", label="Predicted from MoCap")
...
def compare_predicted_forcing_time_series_left_front(mocap_src =r"set_1 - 14-10-19/3kph capture 4.c3d",
    ...
    n_paw = 0 # left paw
    ax.plot(t_range_forceplate[:-1], actual_reactions[:, n_paw]/(m*g), color=colours[0], label="Forceplate measurements", lw = 3)
    ax.plot(t_range_mocap[50:], predicted_mocap_reactions[:, n_paw][50:]/(m*g), color=colours[1], label="Inverse Dynamics predictions", lw=2)
    # ax.set_title(paw_names[n_paw].title())
...
def compare_predicted_forcing_heatmap(mocap_src = r"set_1 - 14-10-19/3kph capture 4.c3d",
    ...
    def heatmap_image(ax, extent, data, direction=0.3, cmap_range=(0, 1), bin_width=5, **kwargs):
        ...
        **kwargs
        ...
        for density in hist: [X.append([density]*10) for i in range(bin_width)] # add a row of density for each row of pixels in the bin
    ...
    for n_paw in range(4):
        ...
        gradient_bar(ax, x=n_paw * sep + width, y_data=predicted_mocap_reactions[:, n_paw][50:], label=paw_names[n_paw], width = width)
