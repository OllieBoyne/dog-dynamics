...
import json
from vis.utils import Vector
import os
from dataset_production.mturk_processor import colours, marker_func
...
def plot_skeleton(ax, data, plot_zeros=False):
	for i, (c1, c2) in skeleton.items():
		for c in [c1, c2]:
			x1, y1 = data[i - 1]
			x2, y2 = data[c - 1]
			if plot_zeros or all(
					i != 0 for i in [x1, x2, y1, y2]):  # Require no non-zero points if plot_zeros flag is False
...
def produce_normalised_skeleton(origin_joint="Nose", target_joint=("Chin")):
	...
	origin_idx, target_idx = keypoint_list.index(origin_joint), keypoint_list.index(
		target_joint)  # get indices of nose, chin
	...
	for i in range(8000):
		...
		angle = - np.arctan2(origin[0] - target[0],
							 - (origin[1] - target[1]))  # - delta y as y coordinates of image are inverted
	...
	for n in range(n_keypoints):
		...
		if n != origin_idx:
			...
			non_zero_locs = all_locs  # chin is centered on (0,0
	...
	plt.tight_layout()
	plt.savefig(
		joinp(fig_dir, f"Normalised skeleton {origin_joint.replace(':', '')} - {target_joint.replace(':', '')}"))
...
def plot_keypoint_distributions():
	...
	sorted_kp = sorted(Y.keys(), key=lambda kp: Y[kp],
					   reverse=True)  # keypoints in descending order of number identified
