...
from my_work.multi_cam_optimiser import *
from my_work.utils import *
from scipy.spatial.transform import Rotation

ordered_colours = ["#d82400"]*3 + ["#fcfc00"]*3 + ["#48b455"]*3 + ["#0090aa"]*3 + ["#d848ff", "#d848ff", "#fc90aa", "#006caa", "#d89000", "#d89000"]
...
def optim(clip = "Gracie Playing", live_plot=False, lr = 0.1, n_it = 2000):
    ...
    for i in range(n_it):
        ...
        if live_plot:
            ...
            if i == 0: ax_loss.set_ylim(0, 1.5*loss.detach().numpy()) # set y lim on first move (assume loss only gets lower)
...
### CURRENT PROBLEM - VARIANCE LOSS CAUSING NAN ERRORS
clip = "Gracie Playing" #"Ally Jump"
# optim(clip=clip, live_plot=False, n_it = 1000)
plot_pred_global(clip)
plot_pred_local(clip)
#
# lrs = [1e-4, 0.001, .005, .01, .05, .1, .5, 1, 2, 3]
# losses = []
# for i in lrs:
#     l = optim(lr=i)
#     losses.append(l.item())
#
# print(*zip(lrs, losses), sep = "\n")