...
from data.data_loader import DataSources
...
from vis.utils import image_to_polygon, PolyArea, fill_segment, try_mkdir
...
dog_test_dataset_1 = dict(
	...
	keypoint_list=["Left front paw", "Left front ankle", "Left front knee", "Left rear paw", "Left rear ankle",
				   "Left rear knee", "Right front paw", "Right front ankle", "Right front knee", "Right rear paw",
				   "Right rear ankle", "Right rear knee", "Tail start", "Tail end", "Base of left ear",
				   "Base of right ear", "Nose", "Chin",
				   ]
)
...
dog_test_dataset_2 = dict(
	...
	keypoint_list=["Left front leg: paw", "Left front leg: middle joint", "Left front leg: top",
				   ...
				   "Right front leg: middle joint", "Right front leg: top", "Right rear leg: paw",
				   "Right rear leg: middle joint",
				   "Right rear leg: top", "Tail start", "Tail end", "Base of left ear", "Base of right ear", "Nose",
				   "Chin"]
)
...
horse_data_set = dict(
	...
	keypoint_list=["Left front leg: hoof", "Left front leg: middle joint", "Left front leg: top",
				   ...
				   "Right front leg: middle joint", "Right front leg: top", "Right rear leg: hoof",
				   "Right rear leg: middle joint",
				   "Right rear leg: top", "Tail start", "Tail end", "Base of left ear", "Base of right ear", "Nose",
				   "Chin"]
)
...
segment_colour_column_names = {
	i: f"Answer.annotatedResult.labelMappings.{i}.color" for i in
	["Left ear tip", "Right ear tip", "Left ear", "Right ear", "Tail"]
}
...
def process_keypoint_data(keypoints_loc, images_dir, keypoint_list, bbox_file_src="", plot_simple=True,
						  plot_detailed=False, minimum_valid_keypoints=8,
	...
	if bbox_file_src != "":
		with open(bbox_file_src, "r") as infile:
			...
			for line in reader:
				...
				if other_data != []:
					full_img_src = f"{other_data[0]}/{img_src}"
				else:
	...
	for img, answers in list(data.items()):
		...
		if img in img_bounds:
			x0, y0, bbox_width, bbox_height = img_bounds[img]
		else:
			x0, y0, bbox_height, bbox_width = 0, 0, *img_data.shape[:2]  # Bbox is whole image if not provided
		...
		for keypoint_label in keypoint_list:
			...
			if not discard:
				...
				image_keypoint_data["joints"].append(
					[mean_x, mean_y, visibility])  # for non entered keypoints, enter simply as 0,0,0
				n_valid_keypoints += 1
				...
				if plot_detailed:
					...
					ax_detailed.plot(mean_x, mean_y, "o", marker=marker_func(keypoint_label), color=colour,
									 label=f"{keypoint_label} [{n_valid}]", ms=2)
...
def view_entries_by_worker(worker_id, img_dir, min_kp=1000):
	...
	with open(results, "r") as infile:
		...
		for n, line in enumerate(reader):
			...
			img, is_multiple_dogs, height, width, answers, this_worker_id, status = get_columns(headers, line, "img",
																								"multiple_dogs",
																								"img_height",
																								"img_width", "answer",
																								"worker", "status")
			...
			if this_worker_id == worker_id and not is_rejected:  # If correct worker and not already rejected
				...
				answers = eval(answers)
				if answers == []:
					answer_dict = {}  # Only store non blank entries
...
def extract_segmentations(images_dir, segment_loc, plot_entries=False, plot_rejects=False, plot_seg=False,
	...
	for i, (img, segments) in enumerate(data.items()):
		...
		if reject_by_correlation:  # Rejection stage considering all 3 entries together
			if len(binary_data) >= 3 or any(s[0] in rejects for s in
											segments):  # Dataset not finished or rejection not updated, only consider data with =3 entries
				...
				for i, j in combs:
					...
					if max_positive < 1:
						...
						correlation = (A * B).sum() / max_positive
...
def filter_keypoints_by_segmentations(**kwargs):
	...
	for n, entry in enumerate(unfiltered):
		...
		joints = [(x, y) for (x, y, v) in entry["joints"] if
				  (x, y) != (0, 0) and v == 1]  # Only count non zero, visible keypoints
		...
		if osp.isfile(seg_path):  # Only consider for acceptance if there is a seg file
			...
			segment = np.dot(rgb_img[..., :3], [0.2989, 0.5870, 0.1140]) > (
					rgb_img.max() / 2)  # load image as binary mask
