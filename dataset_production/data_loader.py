...
class Paw(Rectangle):
	...
	def identify(self, side, end):
		...
		super().__init__((self.x0, self.y0), (1 + self.x1 - self.x0), (1 + self.y1 - self.y0), fill=None, ec=self.colour, lw=2)
	...
	def set_frame(self, f, ax):
		...
		if f < self.frame_start: pass
		elif f == self.frame_start: ax.add_patch(self)
...
class ForcePlateData():
	def __init__(self, src = "", freq=100, n_frames = None, play_every = 1, playback_speed=1.0,
		...
		*_, clips = list(movement)
		...
		for quant in data:
			...
			split_data = (cell_data.replace("\t", "").replace("\n", " ").split(" "))[1:-1] # ignore first and last point as these are blanks
		...
		for paw in paws:
			...
			concurrent_paws = [other for other in paws if overlap(other.frame_start, other.frame_start + len(other.bboxs),
			...
			if end == "front":  mean_x = np.mean([p.x_mean for p in concurrent_paws if p.y_mean <= mean_y])
			else:  mean_x = np.mean([p.x_mean for p in concurrent_paws if p.y_mean >= mean_y])
	...
	def plot_pressure(self, n_frames = None, title="output"):
		...
		save_animation(anim, fig, title=title, frames=len(data), fps = self.fps, dir=r"C:\Users\Ollie\Videos\iib_project\forceplate_vis")
	...
	def save_data(self, data, title=""):
		...
		title = title.replace(r"\\", " -- ").replace(r"/", " -- ").replace("\\", " -- ")[:-4] # Title tells both the set and the name of the file. Also crop out the extension .xml with the [:-4] command.
		np.save(os.path.join(DataSources.forceplate_data, "processed", title), data) # save as title without .xml, and no spaces on the end
...
class MeshData:
	...
	def __init__(self, dir, n_frames = None, freq = 60):
		...
		files = sorted([f for f in os.listdir(full_dir) if ".ply" in f], key=lambda x: int(x.split("_")[1].split(".")[0])) # Sort .ply files by number

		if len(files) == 0: # If files have not been extracted, go through each sub folder to find optimisation for that stage
...
class JointData():
	...
	def set_all_data(self):
		self.all_data = np.swapaxes(np.array([marker.data for marker in self.markers]), 0, 1) # All data as an array of size (n_frames, n_markers, 3)
...
class C3DData(JointData):
	...
	def __init__(self, src = None, ax = None, alt_views = None, interpolate=True, norm = True, crop = None,
		...
		with open(src, 'rb') as handle:
			...
			labels = str(reader.groups["POINT"].params["LABELS"].bytes)[2:] # Extract text of all labels, separated by many spaces
			...
			if fix_rotations:
				for idx in [n for n, i in enumerate(marker_labels) if "paw" in i]:
					...
					for t in troughs:
						...
						next_higher = np.where(vert_data[rot_start+1:] >= vert_data[rot_start] )[0] # next time vert is at a higher value
			...
			if "set_1" in src:
				self.create_averaged_marker("front top", "left front upper", "right front upper") # form new joint at the front top of the system
...
def get_smal_data(smal_npz, smooth = False):
	...
	# for i in [0,1,2]:
	#     ax[i].bar(np.arange(35), means[:, i], yerr=vars.reshape(35,3)[:, i])
	# plt.show()
	# raise ValueError
	# pose_data[:, vars < 0.01] *= 0

	pose = torch.from_numpy(pose_data.reshape(n_frames, 35, 3))
	smalx = smal_deformer_mod.SMALX(opts=None, n_batch=n_frames)

	# turn off paw rotation
	# paw_joints = [10, 20, 14, 24]
	# pose[:, paw_joints] *= 0
	...
	for n, b in enumerate(mean_betas):
		smalx.multi_betas[n] = b

	# smalx.global_rot[:] = pose[:, 0]
	smalx.joint_rot[:] = pose[:, 1:]

	smalx.joint_rot[:, 0] *= 0 # get rid of first motion - LEFT - RIGHT.
	# smalx.joint_rot[:, :, 1] *= 0 # get rid of y-wise rotation
	#
	# all_joints = np.arange(34)
	# joints_keep_all = [i-1 for i in [20, 19, 18, 24, 23, 22]]
	# joints_keep_y_only = all_joints[np.isin(all_joints, joints_keep_all).__invert__()]
	# # #
	# smalx.joint_rot[:, joints_keep_y_only, 0] *= 0 # get rid of x-wise rotation
	# smalx.joint_rot[:, joints_keep_y_only, 2] *= 0  # get rid of z-wise rotation

	# smalx.joint_rot[:, [12-1, 8-1]] *= 0
