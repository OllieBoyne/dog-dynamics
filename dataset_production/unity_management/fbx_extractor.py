...
def extract_first_mesh_from_obj(file, out_dir =r"E:\IIB Project Data\Dog 3D models\all_models\clean_static_meshes", title=None):
    """Given a .obj file, extracts only the first listed mesh, and saves the output in the desired folder, with name = obj filename"""
...
def extract_HP_mesh_from_obj(file, out_dir = r"E:\IIB Project Data\Dog 3D models\all_models\clean_static_meshes", title=None):
    """Given a .obj file, extracts only the first listed mesh, and saves the output in the desired folder, with name = obj filename"""
    ...
    if out == []: return extract_first_mesh_from_obj(file, out_dir, title=title) # extract first if nothing else found
...
def convert_to_npy(src_dir, target_dir=r"E:\IIB Project Data\Dog 3D models\all_models\npy"):
    ...
    TODO address issue here where multiple meshes are stored in the .obj file."""
...
#     convert_to_npy(os.path.join(obj_folder, folder))

# [extract_HP_mesh_from_obj(os.path.join(obj_folder, f, "1.obj")) for f in os.listdir(obj_folder) if "Dach" in f]
...
# ax.disable_mouse_rotation()
# draw_frame(0)
# f = FuncAnimation(fig, draw_frame, frames = 5, interval=20)

# save_animation(anim, fig, frames = 5, fps = 0.2, title="wolf", dir = "E:\IIB Project Data\Blender files\dog runs")